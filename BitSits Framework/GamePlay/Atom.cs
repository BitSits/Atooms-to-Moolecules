...
namespace BitSits_Framework
{
    ...
    class Atom
    {
        ...
        public Atom(Symbol symbol, Vector2 position, GameContent gameContent, World world)
        {
            ...
            bd.position = this.position = position / gameContent.scale;
            bd.bullet = true;
            body = world.CreateBody(bd);
            body.SetUserData(this);

            CircleShape cs = new CircleShape();
            cs._radius = gameContent.atomRadius / gameContent.scale;
        ...
        public void CreateBond()
        {
            ...
            // Mid
            jd.length = (float)(bondLength) / gameContent.scale;
            jd.localAnchorA = jd.localAnchorB = Vector2.Zero;
            Joint midJoint = world.CreateJoint(jd);

            Vector2 a = body.Position * gameContent.scale;
            Vector2 b = nearestAtom.body.Position * gameContent.scale;
            float slope = (float)Math.Atan2((b.Y - a.Y), (b.X - a.X));

            // Short
            jd.length = (float)(bondLength - gameContent.atomRadius * 2) / gameContent.scale;

            Vector2 e = new Vector2((float)Math.Cos(slope), (float)Math.Sin(slope)) * gameContent.atomRadius;

            jd.localAnchorA = body.GetLocalPoint((a + e) / gameContent.scale);
            jd.localAnchorB = nearestAtom.body.GetLocalPoint((b - e) / gameContent.scale);
            ...
            Cue cue = gameContent.soundBank.GetCue("broop"); cue.Play();
        }
        ...
        void JumpAndShake(GameTime gameTime)
        {
            if (symbol == Symbol.Ra && eye == EyeState.Angry)
            {
                for (ContactEdge ce = body.GetContactList(); ce != null; ce = ce.Next)
                {
                    if (ce.Contact.IsTouching())
                    {
                        ...
                        float force = gameContent.random.Next(4);
            ...
            if (bonds.Count == 0)
            {
                if (jumpTime < 0)
                    ...
                    body.ApplyLinearImpulse(new Vector2(0, -0.08f), body.GetWorldCenter());
            }
            ...
            if (eye == EyeState.Wake)
            {
                ...
                position = body.Position +
                    new Vector2((float)Math.Cos(angle), (float)Math.Sin(angle)) * 1f / gameContent.scale;
            }
        ...
        void GetNearestBody()
        {
            ...
            float nearestDist = (bondLength * 1.2f) / gameContent.scale;
        ...
        public void Draw(SpriteBatch spriteBatch, GameTime gameTime)
        {
            ...
            if (symbol == Symbol.Ra || radShockTime < MaxShockTime * 0.5f) 
                radiationSmoke.Draw(spriteBatch);

            spriteBatch.Draw(gameContent.atom[(int)symbol], position * gameContent.scale, null,
                new Color(Color.White, 1 - removeZoom / MaxRemoveTime),
            ...
            if (symbol != Symbol.Ra)
            {
                spriteBatch.DrawString(gameContent.symbolFont, symbolStr, position * gameContent.scale,
                       bondsLeft == 0 ? Color.Gainsboro : Color.White, body.Rotation,
                       symbolCenter, 16f / gameContent.symbolFontSize, SpriteEffects.None, 1);

                spriteBatch.Draw(gameContent.eyes[(int)symbol % 2, (int)eye], position * gameContent.scale,
                    null, Color.White, body.Rotation, gameContent.atomOrigin, 1, SpriteEffects.None, 1);
            }
            else
                spriteBatch.Draw(gameContent.eyes[(int)symbol % 2 - (int)eye, 0], position * gameContent.scale,
                        null, Color.White, body.Rotation, gameContent.atomOrigin, 1, SpriteEffects.None, 1);

            spriteBatch.Draw(gameContent.shine, position * gameContent.scale, null, Color.White,
            ...
            if (eye == EyeState.Shock && electroShockTime < MaxShockTime * 0.15f)
            {
                ...
                animationPlayer.Draw(gameTime, spriteBatch, shockPosition * gameContent.scale);
            }
        }
        ...
        void DrawUnusedBonds(SpriteBatch spriteBatch, int numberOfBonds)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                if (!bondPresent[i])
                {
                    ...
                    spriteBatch.Draw(gameContent.bond[0], body.Position * gameContent.scale, null, Color.White,
