    ...
    class Equipment
    {
        ...
        void SetFixtures()
        {
            ...
            for (int i = 0; i < v.Count - 1; i++)
            {
                ...
                Vector2 c = (v[i] + v[i + 1] - 2 * origin) / 2 / gameContent.scale;
                float h = 8f / 2 / gameContent.scale;
                float w = Vector2.Distance(v[i], v[i + 1]) / 2 / gameContent.scale;
            ...
            {
                float hx = b[i].Width / 2 / gameContent.scale, hy = b[i].Height / 2 / gameContent.scale;
                ps.SetAsBox(hx, hy, (b[i].Position - origin) / gameContent.scale,
            ...
            for (int i = 0; i < circles.Count; i++)
            {
                ...
                cs._radius = circles[i].Diameter / 2 / gameContent.scale;
                cs._p = (circles[i].Position - origin) / gameContent.scale;
        ...
        public Rectangle MoveButtonBound
        {
            get
            {
                ...
                Vector2 pos = body.Position * gameContent.scale;
        ...
        Vector2 RotationButtonPos()
        {
            ...
            theta += body.Rotation;
            Vector2 pos = body.Position * gameContent.scale + new Vector2((float)Math.Cos(theta),
        ...
        public void DrawBottomLayer(SpriteBatch spriteBatch)
        {
            ...
            if (isSelected)
                spriteBatch.Draw(gameContent.equipment[(int)equipName, 2], body.Position * gameContent.scale, null,
                    Color.White, body.Rotation, origin - topLeftVertex, 1, SpriteEffects.None, 1);

            if (isClamped) DrawClamps(spriteBatch);

            spriteBatch.Draw(gameContent.equipment[(int)equipName, 0], body.Position * gameContent.scale, null,
                Color.White, body.Rotation, origin - topLeftVertex, 1, SpriteEffects.None, 1);
        ...
        void DrawClamps(SpriteBatch spriteBatch)
        {
            ...
            {
                Vector2 position = body.Position * gameContent.scale;
            ...
            spriteBatch.Draw(gameContent.clamp, body.Position * gameContent.scale + pos, null, Color.White,
        ...
        void DrawpHvalue(SpriteBatch spriteBatch)
        {
            ...
            spriteBatch.Draw(gameContent.pointer,
                body.Position * gameContent.scale + new Vector2(100, 0) - gameContent.pointerOrigin,
                Color.White);

            spriteBatch.DrawString(gameContent.symbolFont, pHvalue.ToString("0.00"),
                body.Position * gameContent.scale + new Vector2(140, -20),
        ...
        {
            spriteBatch.Draw(gameContent.equipment[(int)equipName, 1], body.Position * gameContent.scale, null,
                Color.White, body.Rotation, origin - topLeftVertex, 1, SpriteEffects.None, 1);

            if (editMode)
            {
                Color c = new Color(Color.White, 0.65f);
                if (isMouseOver) c = new Color(Color.White, 1f); 

                spriteBatch.Draw(gameContent.equipMoveButton, body.Position * gameContent.scale, null,
